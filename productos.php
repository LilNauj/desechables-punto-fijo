...
$sql = "SELECT p.*, c.nombre as categoria_nombre, c.id as categoria_id,
        CASE 
            WHEN p.tiene_variantes = 1 THEN (SELECT MIN(precio) FROM producto_variantes WHERE producto_id = p.id AND estado = 'disponible')
            ELSE p.precio
        END as precio_mostrar,
        CASE 
            WHEN p.tiene_variantes = 1 THEN (SELECT SUM(stock) FROM producto_variantes WHERE producto_id = p.id)
            ELSE p.stock
        END as stock_total
        FROM productos p 
...
// Obtener variantes para productos que las tengan
foreach ($productos as &$producto) {
    if ($producto['tiene_variantes'] == 1) {
        $stmt = $db->prepare("SELECT * FROM producto_variantes WHERE producto_id = ? AND estado = 'disponible' ORDER BY es_variante_principal DESC, precio ASC");
        $stmt->bind_param("i", $producto['id']);
        $stmt->execute();
        $producto['variantes'] = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        $stmt->close();
    } else {
        $producto['variantes'] = [];
    }
...
<body class="index-page">
                        ...
                        <div class="product-showcase">
                            <div class="product-card">
                                ...
                                <div class="hero-product-image">
                                    <img src="assets/img/banner-productos.png"
                        ...
                        <div class="row gy-4" id="gridProductos">
                            <?php foreach ($productos as $producto): ?>
                                <div class="col-lg-4 col-md-6 product-card"
                                    ...
                                    data-precio="<?php echo $producto['precio_mostrar']; ?>"
                                        ...
                                        <div class="product-details">
                                            ...
                                            <div class="product-meta">
                                                <div class="product-price">
                                                    <?php if ($producto['tiene_variantes'] == 1 && !empty($producto['variantes'])): ?>
                                                        Desde $<?php echo number_format($producto['precio_mostrar'], 0); ?>
                                                    <?php else: ?>
                                                ...
                                                <div class="product-rating">
                                                    <i class="bi bi-star-fill"></i>
                                                    <span>(<?php echo $producto['stock_total']; ?>)</span>
                                                </div>
                                            </div>
                                            ...
                                            <?php if ($producto['tiene_variantes'] == 1 && !empty($producto['variantes'])): ?>
                                                <div class="mb-2">
                                                    <small class="text-muted">
                                                        ...
                                                        <?php echo count($producto['variantes']); ?> variante(s) disponible(s)
                                                    </small>
                                                </div>
                                            <?php endif; ?>
                                            ...
                                            <?php if ($producto['stock_total'] > 0): ?>
                                                <?php if ($producto['tiene_variantes'] == 1 && !empty($producto['variantes'])): ?>
                                                    <button class="btn btn-primary w-100 mt-3"
                                                        onclick="mostrarVariantes(<?php echo $producto['id']; ?>, '<?php echo htmlspecialchars($producto['nombre']); ?>', <?php echo htmlspecialchars(json_encode($producto['variantes'])); ?>)">
                                                        <i class="bi bi-collection"></i> Seleccionar Variante
                                                    </button>
                                                <?php else: ?>
                                                    <button class="btn btn-primary w-100 mt-3"
                                                        onclick="agregarAlCarrito(<?php echo $producto['id']; ?>, null, '<?php echo htmlspecialchars($producto['nombre']); ?>')">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" id="modal_producto_nombre">

                    <h6 id="modal_titulo_producto" class="mb-3"></h6>

                    <div id="lista_variantes" class="list-group">
    ...
    <script>
        ...
        function agregarAlCarrito(productoId, varianteId, productoNombre) {
            ...
            if (varianteId) {
                ...
                inputVariante.name = 'variante_id';
        ...
        function mostrarVariantes(productoId, productoNombre, variantes) {
            ...
            document.getElementById('modal_titulo_producto').textContent = productoNombre;

            const listaVariantes = document.getElementById('lista_variantes');
            ...
            <br>
            <small class="text-muted">Stock: ${variante.stock}</small>
        `;

                const precio = document.createElement('span');
                precio.className = 'badge bg-primary rounded-pill fs-6';
                precio.textContent = '$' + Number(variante.precio).toLocaleString('es-CO');
                ...
                if (variante.stock > 0) {
                    div.onclick = function () {
                        agregarAlCarrito(productoId, variante.id, productoNombre + ' - ' + variante.nombre_variante);
                    };
                } else {
                    div.classList.add('disabled');
                    div.style.opacity = '0.5';
                    const agotado = document.createElement('span');
                    agotado.className = 'badge bg-secondary';
